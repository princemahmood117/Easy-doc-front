...

                {
                    ...
                    <div className='flex items-center gap-2 cursor-pointer group relative'>
                        ...
                        <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                            <div className='min-w-48 bg-stone-200 rounded flex-col gap-4 p-4' >
                    ...
                    :
                    <button onClick={()=> navigate('/login')} className='bg-primary text-white px-4 py-3 rounded-full font-light hidden md:block '>Create account</button>
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-300`}>
                    ...
                    <ul className='flex flex-col items-center gap-3 mt-5 px-5 text-lg font-medium'>
                        <NavLink  onClick={()=> setShowMenu(false)} to='/'>
                            <p className='px-4 py-2 incline-block rounded'>Home</p>
                        </NavLink>

                        <NavLink  onClick={()=> setShowMenu(false)} to='doctors'>
                            <p className='px-4 py-2 incline-block rounded'>All Doctors</p>
                        </NavLink>

                        <NavLink  onClick={()=> setShowMenu(false)} to='/about'>
                            <p className='px-4 py-2 incline-block rounded'>About</p>
                        </NavLink>

                        <NavLink  onClick={()=> setShowMenu(false)} to='/contact'>
                            <p className='px-4 py-2 incline-block rounded'>Contact</p>
                        </NavLink>

                        <NavLink  onClick={()=> setShowMenu(false)} to='/login'>
                            <p className='px-4 py-2 incline-block rounded'>Login</p>
                        </NavLink>
                    </ul>
                </div>
